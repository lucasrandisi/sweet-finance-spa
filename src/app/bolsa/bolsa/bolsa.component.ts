...
export class BolsaComponent implements OnInit {
	...
	draw(ticker : any){
		...
		//request for RSI
		this.http.get(`${environment.apiEndpoint}/twelve-data/rsi`,{
			params: {
				symbol: ticker,
				interval: '1day',
				outputsize: '90'
		...
		//request for MACD
		this.http.get(`${environment.apiEndpoint}/twelve-data/macd`,{
			params: {
				symbol: ticker,
				interval: '1day',
				outputsize: '90'
	...
	get_favorites(){
		this.http.get(`${environment.apiEndpoint}/stocks/favorites`,{
			params: {
			}
		}).subscribe((response:any)=>{
			this.favorites = [];
			...
			for(let f of response){
				let observable1 = this.http.get(`${environment.apiEndpoint}/twelve-data/price`,{
					params: {
					symbol: f.stock_symbol
					}
				});
		
				let observable2 = this.http.get(`${environment.apiEndpoint}/twelve-data/quote`,{
					params: {
					symbol: f.stock_symbol
					}
				});
	...
	add_favorite(){
		...
		this.http.post(`${environment.apiEndpoint}/stocks/favorites`,{
			stock_symbol: ticker
		}).subscribe((response:any)=>{
	...
	delete_favorite(id : any){
		this.http.delete(`${environment.apiEndpoint}/stocks/favorites/${id}`,{})
		.subscribe((response:any)=>{
			let c = 0;
			for (let e of this.favorites){
				if (e.id == id){
